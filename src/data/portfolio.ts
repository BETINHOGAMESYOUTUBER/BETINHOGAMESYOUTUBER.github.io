import { PortfolioProject } from '@/types';

export const portfolioProjects: PortfolioProject[] = [
  {
    id: 'projeto-01',
    title: 'Projeto Rosto Digital Personalizado PES 2021',
    category: 'Rosto Personalizado',
    description: 'Transformação de fotografia real em modelo 3D com texturas customizadas e proporções faciais ajustadas.',
    coverImage: '/images/project-01.jpg',
    referenceImage: '/images/learn-reference-photo.jpg',
    processImage: '/images/learn-facial-modeling.jpg',
    ingameImage: '/images/learn-pes-application.jpg',
    platform: 'PES 2021 PC',
    tags: ['FaceMaker 3D', 'Textura Realista', 'Fotogrametria', 'PC'],
  },
  {
    id: 'projeto-02',
    title: 'Projeto Atleta Amador Edição Sob Encomenda',
    category: 'PES 2021 PC',
    description: 'Modelagem de atributos e fisionomia para integração de jogador em modo Carreira / Master Liga.',
    coverImage: '/images/project-02-cover.jpg',
    referenceImage: '/images/project-02-ref.jpg',
    processImage: '/images/project-02-mesh.jpg',
    ingameImage: '/images/project-02-game.jpg',
    platform: 'PES 2021 PC',
    tags: ['Master Liga', 'Face Custom', 'Alinhamento 3D'],
  },
  {
    id: 'projeto-03',
    title: 'Projeto Player Pro PES 2021 PS4 Desbloqueado',
    category: 'PES 2021 PS4 Desbloqueado',
    description: 'Adaptação de características faciais marcantes com foco em compatibilidade para a versão de console PS4 Desbloqueado.',
    coverImage: '/images/project-03-cover.jpg',
    referenceImage: '/images/project-03-ref.jpg',
    processImage: '/images/project-03-mesh.jpg',
    ingameImage: '/images/project-03-game.jpg',
    platform: 'PES 2021 PS4 Desbloqueado',
    tags: ['PS4 Desbloqueado', 'OptiFile', 'Fidelidade Visual'],
  },
  {
    id: 'projeto-04',
    title: 'Projeto Criador de Conteúdo Avatar Gamer 3D',
    category: 'Rosto Personalizado',
    description: 'Reconstrução digital completa para streaming e criação de conteúdo gamer no PES 2021.',
    coverImage: '/images/project-04-cover.jpg',
    referenceImage: '/images/project-04-ref.jpg',
    processImage: '/images/project-04-mesh.jpg',
    ingameImage: '/images/project-04-game.jpg',
    platform: 'PES 2021 PC',
    tags: ['Streamer Avatar', 'PES 2021', 'Alta Fidelidade'],
  },
];
